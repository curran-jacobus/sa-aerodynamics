...
def calc_CD0_regression(airfoilfile, re_list=np.linspace(1e5,1e6,10)):
    ...
    cd0_list=[]
    for re in re_list:
        cd0_list.append(get_CD0(re,os.path.join(os.getcwd(),"airfoils",airfoilfile)))
...
for airfoil_file in os.listdir(os.path.join(os.getcwd(),'airfoils')):
    try:
        prefactor, expofactor = calc_CD0_regression(airfoil_file)
        print(airfoil_file)
        with open('airfoil_cd0.csv', mode='a') as file:
            writer= csv.writer(file, delimiter=",")
            writer.writerow([airfoil_file,prefactor,expofactor])
    except:
        os.remove(os.path.join(os.getcwd(),"airfoils",airfoil_file))
